...
fun RotationLabScreen(factory: AppViewModelFactory) {
    ...
    val gradiente = LabGradient.ROTACION
    ...
    Box(modifier = Modifier.fillMaxSize().background(Brush.verticalGradient(AppBackgroundGradient))) {
        Column(
            ...
            Text(
                gradiente.emoji + " " + stringResource(R.string.lab_rotacion_titulo),
                style = MaterialTheme.typography.headlineMedium,
                fontWeight = FontWeight.ExtraBold
            )
            Text(
                stringResource(R.string.lab_rotacion_instruccion),
            ...
            if (state.midiendo) {
                LiveMeasuringBadge(texto = "Midiendo el giro…", color = gradiente.colors.first(), modifier = Modifier.padding(top = 12.dp))
            }
            ...
            if (state.historialMagnitud.isNotEmpty()) {
                ...
                SimpleLineChart(values = state.historialMagnitud, lineColor = gradiente.colors.first(), modifier = Modifier.padding(top = 4.dp))
            }
